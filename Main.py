...
pygame.init()  # "pygame.init()" starts pygame, essential for it to work, helps render images and sounds
screen = pygame.display.set_mode((800, 400))  # Sets up screen
pygame.display.set_caption('Runner')  # Changes the title of screen
clock = pygame.time.Clock()  # Sets up the clock
test_font = pygame.font.Font('Pixel-type.ttf', 50)
game_active = False
start_time = 0
score = 0
...
# Snail
snail_frame_1 = pygame.image.load('snail1.png').convert_alpha()  # Converts png into a better format for pygame
...
player_rect = player_surface.get_rect(midbottom=(80, 300))  # Makes a rect around player
player_gravity = 0
...
while True:  # Runs game forever
    for event in pygame.event.get():  # Gets event or any input from user
        if event.type == pygame.QUIT:  # Makes the screen able to be closed
            pygame.quit()  # Actually makes the program close
            exit()  # Exits the while True loop
    ...
    if game_active:
        screen.blit(sky_surface, (0, 0))  # Draws the surface on the screen
    ...
    clock.tick(60)  # Limits the while True loop to running 60 times per second
